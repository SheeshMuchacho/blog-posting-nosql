...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.email.trim()) {
      ...
      newErrors.email = t(
        { en: 'Please enter a valid email', ja: '有効なメールアドレスを入力してください', ko: '유효한 이메일을 입력해주세요' },
        lang
      );
    }
  ...
  // Always render; toggle interactivity & transitions via classes for smooth enter/exit
  return (
    <div
      className={[
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {submitStatus === 'success' && (
            <div className="bg-green-100/80 border border-green-200 text-green-800 px-4 py-3 rounded-lg backdrop-blur-sm">
              {t(
                {
                  en: "Thank you for your message! We'll get back to you soon.",
                  ja: 'メッセージありがとうございます！すぐにご連絡いたします。',
          ...
          {submitStatus === 'error' && (
            <div className="bg-red-100/80 border border-red-200 text-red-800 px-4 py-3 rounded-lg backdrop-blur-sm">
              {t(
                {
                  en: 'Something went wrong. Please try again.',
                  ja: '何かが間違っています。もう一度お試しください。',
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            ...
            >
              {isSubmitting ? (
                ...
                t({ en: 'Send Message', ja: 'メッセージを送信', ko: '메시지 보내기' }, lang)
              )}
            </button>
          </div>
        </form>
